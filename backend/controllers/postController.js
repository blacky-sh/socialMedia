...
const createPost = async (req, res) => {
  try {
    const { postedBy, text, category, propertyType } = req.body;
    let { img } = req.body;
    ...
    const newPost = new Post({ postedBy, text, img, category, propertyType });
    await newPost.save();
...
const getFeedPosts = async (req, res) => {
  try {
    ...
    const feedPosts =
      await Post.find(/*{ postedBy: { $in: following } }*/).sort({
        createdAt: -1,
      });
...
const markAsFound = async (req, res) => {
  try {
    ...
    if (post.postedBy.toString() !== userId.toString()) {
      return res
        .status(401)
        .json({ error: "Unauthorized to mark this post as found" });
    }
    ...
    res
      .status(200)
      .json({ message: "Post marked as found successfully", post });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
